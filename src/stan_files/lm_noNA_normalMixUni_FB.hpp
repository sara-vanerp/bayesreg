...
namespace model_lm_noNA_normalMixUni_FB_namespace {
...
stan::io::program_reader prog_reader__() {
    ...
    reader.add_event(0, 0, "start", "model_lm_noNA_normalMixUni_FB");
    reader.add_event(47, 45, "end", "model_lm_noNA_normalMixUni_FB");
    return reader;
}

#include <meta_header.hpp>
 class model_lm_noNA_normalMixUni_FB : public prob_grad {
private:
    int N_train;
    int p;
    ...
    matrix_d X_test;
public:
    model_lm_noNA_normalMixUni_FB(stan::io::var_context& context__,
    ...
    void ctor_body(stan::io::var_context& context__,
        ...
        static const char* function__ = "model_lm_noNA_normalMixUni_FB_namespace::model_lm_noNA_normalMixUni_FB";
        ...
        try {
            ...
            validate_non_negative_index("tau2", "p", p);
            num_params_r__ += p;
    ...
    void transform_inits(const stan::io::var_context& context__,
        ...
        pos__ = 0U;
        validate_non_negative_index("tau2", "p", p);
        context__.validate_dims("initialization", "tau2", "vector_d", context__.to_vec(p));
        vector_d tau2(static_cast<Eigen::VectorXd::Index>(p));
        for (int j1__ = 0U; j1__ < p; ++j1__)
        ...
        pos__ = 0U;
        validate_non_negative_index("gamma", "p", p);
        context__.validate_dims("initialization", "gamma", "vector_d", context__.to_vec(p));
        vector_d gamma(static_cast<Eigen::VectorXd::Index>(p));
        for (int j1__ = 0U; j1__ < p; ++j1__)
            gamma(j1__) = vals_r__[pos__++];
        try {
            writer__.vector_lub_unconstrain(0,1,gamma);
        } catch (const std::exception& e) { 
    ...
    T__ log_prob(vector<T__>& params_r__,
        ...
        try {
            ...
            Eigen::Matrix<local_scalar_t__,Eigen::Dynamic,1>  tau2;
            (void) tau2;  // dummy to suppress unused var warning
            if (jacobian__)
                tau2 = in__.vector_lb_constrain(0,p,lp__);
            else
                tau2 = in__.vector_lb_constrain(0,p);

            Eigen::Matrix<local_scalar_t__,Eigen::Dynamic,1>  gamma;
            (void) gamma;  // dummy to suppress unused var warning
            if (jacobian__)
                gamma = in__.vector_lub_constrain(0,1,p,lp__);
            else
                gamma = in__.vector_lub_constrain(0,1,p);
            ...
            lp_accum__.add(inv_gamma_log<propto__>(tau2, 0.5, 0.5));
            ...
            for (int j = 1; j <= p; ++j) {
                ...
                lp_accum__.add(log_sum_exp((stan::math::log(get_base1(gamma,j,"gamma",1)) + normal_log(get_base1(beta,j,"beta",1),0,stan::math::sqrt(0.001))),(stan::math::log((1 - get_base1(gamma,j,"gamma",1))) + normal_log(get_base1(beta,j,"beta",1),0,stan::math::sqrt(get_base1(tau2,j,"tau2",1))))));
            }
    ...
    void write_array(RNG& base_rng__,
        ...
        vars__.resize(0);
        stan::io::reader<local_scalar_t__> in__(params_r__,params_i__);
        static const char* function__ = "model_lm_noNA_normalMixUni_FB_namespace::write_array";
        (void) function__;  // dummy to suppress unused var warning
        // read-transform, write parameters
        double mu = in__.scalar_constrain();
        double sigma2 = in__.scalar_lb_constrain(0);
        vector_d beta = in__.vector_constrain(p);
        vector_d tau2 = in__.vector_lb_constrain(0,p);
        vector_d gamma = in__.vector_lub_constrain(0,1,p);
        vars__.push_back(mu);
    ...
    static std::string model_name() {
        return "model_lm_noNA_normalMixUni_FB";
    }
...
typedef model_lm_noNA_normalMixUni_FB_namespace::model_lm_noNA_normalMixUni_FB stan_model;
