...
#' @param missing Logical value indicating whether the data contains missing values. Currently only supported in combination with the ridge prior.
...
stan_reg_lm <- function(X, y, N_train, 
	...
	missing = c(TRUE, FALSE), df = 0.5, p0 = NA, global_scale = 1, global_df = 1, local_df = 1,
	...
	if(missing == TRUE){
		if(prior != "ridge"){ stop("Missing data is currently only supported in combination with the ridge prior.") }
		if(prior == "ridge"){
			mod = paste0("lm_NA_", prior, "_FB")
		}

		# standardize the predictor matrix before NAs are transformed to magic numbers
		stanX = scale(X)

		# transform NAs to -99999
		stanX[which(is.na(stanX))] = -99999
		y[which(is.na(y))] = -99999
		...
		ntest = nrow(stanX) - N_train
		xtest = stanX[(N_train + 1):nrow(stanX), ]
		idmis_ytrain = as.array(which(ytrain == -99999))
		nmis_ytrain = length(idmis_ytrain)
		idmis_xtrain = which(xtrain == -99999, arr.ind = TRUE)
		nmis_xtrain = nrow(idmis_xtrain)
		idmis_xtest = which(xtest == -99999, arr.ind = TRUE)
		nmis_xtest = nrow(idmis_xtest)

		# create stan input & run
		standata <- list(N_train = N_train, p = npred, y_train = ytrain, X_train = xtrain, N_test = ntest, X_test = xtest,
			N_mis_trainY = nmis_ytrain, ind_mis_trainY = idmis_ytrain, N_mis_trainX = nmis_xtrain, ind_mis_trainX = idmis_xtrain,
			N_mis_testX = nmis_xtest, ind_mis_testX = idmis_xtest, sd_imp = 1)
	}
